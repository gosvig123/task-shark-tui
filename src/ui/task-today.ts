...
export function removeDemoToday(view: View, task: Task): string {
  if (task.ownerList === todayList) return 'Demo date saved; task stored directly in Today was kept there.';
...
export async function recoverToday(view: View, config: Config, task: Task): Promise<void> {
  ...
  const result = await removeTodayReference(config.tasks, task, true);
