...
export type WorkspaceSection = 'Details' | 'Conversations';
...
export function workspaceSection(view: View, section: WorkspaceSection): void {
  ...
  if (section === 'Details' && !view.boards.state(view.taskScope.id).loaded) void view.boards.load(view.taskScope.id);
}
export function workspaceContent(view: View): string {
  ...
  if (view.workspaceSection === 'Details') return `${safe(taskDetails(task, []))}\n\n${boardContent(view)}`;
...
function boardContent(view: View): string {
  ...
  const lines = [`Board Updates · ${count}${s.earlier ? '+' : ''} unread`,
    `All progress · ${s.entries.length} updates · all kinds`,
  ...
  if (s.earlier) lines.push('Earlier updates exist. Press b to load the previous page.', '');
  if (s.pending) lines.push('Uncertain submission retained. Press u to retry safely with the same request ID.', safe(s.pending.body), '');
  if (s.error) lines.push(safe(s.error), '');
  if (!s.entries.length) lines.push(s.loaded ? 'No Board Updates. Press u to post.' : 'Press f to load Board Updates.');
  if (s.entries.length && !canReview(s) && count) lines.push('Review disabled: earlier unread entries are not shown. Press b to load them before review.');
