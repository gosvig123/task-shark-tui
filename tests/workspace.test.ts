...
import { openWorkspace, closeWorkspace, workspaceContent, workspaceSection } from '../src/ui/workspace.js';
...
test('opening a task keeps source selection and filters for back, not just row selection', () => {
...
test('workspace keys keep board review separate and preserve literal untrusted content', async () => {
  ...
  await workspaceKey(view, 'escape'); assert.equal(view.taskScope, undefined);
});
test('highlighting a conversation previews without review; Enter opens and Escape returns left then task list', async () => {
  const { task, view } = fixture();
  ...
  await workspaceKey(view, 'escape'); assert.equal(view.workspaceFocus, 'left'); assert.equal(view.taskScope, task);
  await workspaceKey(view, 'escape'); assert.equal(view.taskScope, undefined); assert.equal(view.selected, 'Source/task');
});
