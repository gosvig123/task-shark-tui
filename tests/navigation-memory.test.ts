...
import { openWorkspace, closeWorkspace } from '../src/ui/workspace.js';
...
test('task workspace state restores as preview without invoking helper reads or review', () => {
  ...
  closeWorkspace(view); assert.equal(view.query, 'calendar'); assert.equal(view.listFilter, 'Work');
...
test('separate task workspaces do not leak their selected section or Board entry', () => {
  ...
  openWorkspace(view, other); assert.equal(view.workspaceSection, 'Details'); assert.equal(view.boardSequence, undefined);
  view.workspaceSection = 'Conversations'; closeWorkspace(view);
