...
test('Task Workspace layout: wider selection column and adaptive panels at all window sizes', () => {
  assert.equal(taskNavigationWidth(100), 40); assert.equal(taskNavigationWidth(200), 80);
  assert.deepEqual(taskNavigationHeights(28), [25, 3]);
  assert.deepEqual(taskNavigationHeights(10), [7, 3]);
